...
\end{itemize}
\end{textblock*}
\begin{textblock*}{12cm}(1cm,6.5cm)
\begin{itemize}
\item{Nicht-Blockend}
\begin{itemize}
...
\begin{frame}{\subsecname: \subsubsecname}
\begin{itemize}
\item{Vor Iteration des Algorithmus: configurierbarer Rolling Average Filter}
\item{Während des Algorithmus:\\
Akkumulation bis zur nächsten Punktwolke}
\end{itemize}
\begin{center}
\begin{tikzpicture}
\draw (0, 0) rectangle +(1, 1) node[pos=0.5] {IMU};
\draw[->] (1, 0.5) -- (3, 0.5);
\draw[fill=light] (1.75, 0.25) rectangle +(0.5, 0.5);
\node[above] at (2, 0.75) {Pose};
...
\draw (7, -1) rectangle +(3, 1) node[pos=0.5] {Registration};
\end{tikzpicture}
\end{center}
\end{frame}
...
\foreach \i\y in {0/3,1/11,2/15,3/10,4/7,5/3,6/2,7/1,8/1,9/1} {
    \pgfmathsetmacro\res{int(\i * 10)}
    \draw (\i, -0.1) node[below]{\res} -- (\i, 0.1);
    \tikzmath {
        if {\i < 5} then {
            let \c = light;
        } else {
...
\section{Ausblick bis Hauptvortrag}
\begin{frame}{\secname}
\begin{itemize}
\item{Aufnahme eines Teils der Innenstadt auf Kettcar}
\item{Aufarbeitung theoretischer Hintergründe (Registrierung, Kartierung, etc...)}
\item{Vervollständigung der Dokumentation und static code checking}
\end{itemize}
\end{frame}
