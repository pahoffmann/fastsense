...
#pragma HLS loop_tripcount max=NUM_POINTS/SPLIT_FACTOR*128*2
        ...
        /*if(tsdf_value < -30000000 && map.in_bounds(current_cell.x, current_cell.y, interpolate_z))
        {
            std::cout << "Wrong calc at " << map.getIndex(current_cell.x, current_cell.y, interpolate_z)
            << " " << tsdf_value
            << " " << (current_point - current_cell.to_mm()).x
            << " " << (current_point - current_cell.to_mm()).y
            << " " << (current_point - current_cell.to_mm()).z
            << " " << (current_point - current_cell.to_mm()).norm2() << "\n";
        }*/
...
#pragma HLS dependence variable=new_entries inter false
            ...
            /*if(entry.first < -300 || entry.first > 300)
            {
                std::cout << "Wrong read at " << map.getIndex(current_cell.x, current_cell.y, interpolate_z) << "\n";
            }*/
            IntTuple new_entry;
...
int main()
{
    ...
    test::test_krnl_tsdf(points, points, 360, mapData_test, mapData_test, 21, 21, 5, 0, 0, 0, 10, 10, 10, newEntries_test, newEntries_test, 300, 10);
