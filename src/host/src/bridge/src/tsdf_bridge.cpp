...
bool inBounds(int x, int y, int z, fs::map::LocalMapPos& pos, fs::map::LocalMapSize& size) const
{
    return abs(x - pos.x()) <= size.x() / 2 && abs(y - pos.y()) <= size.y() / 2 && abs(z - pos.z()) <= size.z() / 2;
}

// TODO in util, so steffen and my 
std::pair<float, float> get_tsdf_value(std::array<std::pair<float, float>>& tsdf_data, int x, int y, int z, fs::map::LocalMapPos& pos, fs::map::LocalMapSize& size, fs::map::LocalMapOffset& offset)
{
    ...
    std::pair<float, float> tsdf_value = tsdf_data[((x - pos.x() + offset.x() + size.x()) % size.x()) * size.y() * size.z() +
...
int main(int argc, char** argv)
{
    ...
    while (ros::ok())
    {
        ...
        fs::map::LocalMapSize size = msg.size_;
        fs::map::LocalMapPos pos = msg.pos_;
        fs::map::LocalMapOffset offset = msg.offset_;
        ...
        #pragma omp parallel num_threads(thread_count)
        {
            ...
            for (int x = left[0]; x <= right[0]; x++)
            {
                for (int y = left[1]; y <= right[1]; y++)
                {
                    ...
                    {
                        auto& val = get_tsdf_value(msg.tsdf_data_, x, y, z, pos, size);
