...
void update_tsdf(const ScanPoints_t& scan_points,
                 ...
                 int max_weight)
{
    float angle = 30.0 / 15; // TODO: Scanner FoV as Parameter
    ...
    std::vector<std::unordered_map<Vector3i, std::pair<int, int>>> values(thread_count);

    //#pragma omp parallel num_threads(thread_count)
    {
        int current_thread = 0;
        auto& local_values = values[current_thread];
        std::unordered_set<Vector3i> visited;
